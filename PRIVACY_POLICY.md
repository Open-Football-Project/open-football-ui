# Open Football Project Privacy Policy

*Last updated: March 14, 2026*

Open Football Project ("the App") is a football match information app. This Privacy Policy explains our data practices.

## Data Collection

We do not collect, store, or share any personal data from users. The App does not require account registration or login.

## Third-Party Data

Match and football data is sourced from API-Football (api-football.com). We do not control their data practices. Please refer to their privacy policy at https://www.api-football.com/privacy for details.

## Children

This App is not directed at children under 13 and does not knowingly collect data from children.

## Contact

For any questions regarding this Privacy Policy, contact us at: open.football.project@gmail.com
