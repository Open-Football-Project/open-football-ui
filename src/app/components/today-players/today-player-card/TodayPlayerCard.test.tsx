...
const renderCard = (props: Parameters<typeof TodayPlayerCard>[0]) =>
  render(
    <MemoryRouter>
      <TodayPlayerCard {...props} />
    </MemoryRouter>,
  );
...
describe("TodayPlayerCard", () => {
  it("renders the player's photo, name, position, and team", () => {
    renderCard({ playerScore, teamName: "Argentina" });
  ...
  it("truncates the markets list to 2 plus a '+N more' summary when there are more than 3 matched markets, capping the card at 4 lines like a Season Form card", () => {
    ...
    renderCard({ playerScore: manyMarketsScore, teamName: "Argentina" });
  ...
  it("shows a Season Form signal badge and one list item per stat when the signal is SEASON_STAT", () => {
    ...
    renderCard({ playerScore: seasonStatScore, teamName: "Argentina" });
  ...
  it("links to the player's profile page when the player has an id", () => {
    renderCard({ playerScore, teamName: "Argentina" });

    const link = screen.getByRole("link", { name: "L. Messi player history" });
    expect(link).toHaveAttribute("href", "/player/154");
  });

  it("does not render a link when the player has no id", () => {
    renderCard({
